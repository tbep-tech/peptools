...
A different year, parameter, and bay segment can also be chosen. 
...
The results from `show_matrixpep()` and `show_wqmatrixpep()` can be combined for an individual segment using the `show_segmatrixpep()` function.  This is useful to understand which water quality parameter is driving the outcome for a given year. The plot shows the secchi and chlorophyll outcomes from `show_wqmatrixpep()` next to the segment management outcomes from `show_matrixpep()`.  Only one segment can be plotted for each function call. 
