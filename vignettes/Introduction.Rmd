...
First, install the devtools package, load devtools, then install and load peptools.  Note that peptools only needs to be installed once, but it needs to be loaded every new R session (i.e., `library(peptools)`).

```{r, eval = F}
install.packages('devtools')
library(devtools)
install_github('tbep-tech/peptools')
library(peptools)
```
