...
Begin by installing the package from GitHub.  The source code is available on the tbep-tech GitHub group web page: <https://github.com/tbep-tech/peptools>.  

First, install the remotes package and then install peptools.  We need the remotes package to install peptools from our GitHub site.  Note that peptools only needs to be installed once, but it needs to be loaded every new R session (i.e., `library(peptools)`).
...
The following shows all functions provided in peptools, organized by prefix. 
...
The `show_sitemappep()` function produces an interactive map of medians of water quality conditions at each station.  Medians can be shown for chlorophyll or secchi depth and for all stations or only stations from selected bay segments.  Each point on the map shows the median for the parameter, with the size and color of the point in proportion to the other median values shown on the map.  The color scale for the median shows higher concentrations of chlorophyll or shallower secchi depths in red and lower concentrations of chlorophyll or deeper secchi depths in blue.  Hovering the mouse pointer over a site location will indicate the site name and the median value.  Clicking on a station point will reveal the underlying plot data. 

Here, the 2020 chlorophyll medians are shown for stations in all bay segments. 

```{r}
show_sitemappep(dat, yrsel = 2020)
```
...
```{r}
show_sitemappep(dat, yrsel = 2010, param = 'sd', bay_segment = c('Central', 'Eastern'))
```
...
```{r}
show_sitemappep(dat, yrsel = 2020, relative = T)
```
...
```{r}
show_sitemappep(dat, yrsel = 2020, relative = T, maxrel = 0.8)
```
...
```{r, fig.height = 5, fig.width = 8}
show_thrpep(dat, bay_segment = "Western", param = "chla", yrrng = c(1976, 2020))
```
...
show_matrixpep(dat, txtsz = NULL) +
  scale_y_continuous(expand = c(0,0), breaks = c(1976:2020)) + 
  coord_flip() +
