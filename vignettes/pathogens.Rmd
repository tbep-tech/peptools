...
The `anlz_entpep()` function estimates this exposure risk by counting the number of instances in a year when concentrations at a beach exceeded the threshold for each 24 hour period in the dataset.  The results show number of days as `samples` for each year, number of `exceedances` for the samples, and `perexceedances` as the proportion of samples wiht exceedances. Only the 28 relevant beaches for the Peconic Estuary are returned.
...
```{r, fig.height = 7, fig.width = 7, fig.cap = 'Frequencies of exceedances of *Enterococcus* concentrations at Peconic Estuary bathing beaches for the years 2010 through 2020.  Different options are used for the threshold and color categorization.'}
