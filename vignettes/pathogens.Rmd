...
The function `anlz_entpep()` summarizes the imported data at each beach to quantify the number of closures each year. Closures were identified by the number of instances in a year when *Enterococcus* exceeded a 104 cfu/100 ml criterion used by SFDHS. Note that this threshold is based on a single-sample limit at each collection date/time, whereas alternative thresholds could be used based on 30-day averages. Exceedances at each beach within a year were counted as the number of days when any of the concentrations across the observations in `FieldNum` exceeded the threshold.  This avoids double-counting exceedances within a day if more than one value in `FieldNum` exceeded the threshold for the same date.
